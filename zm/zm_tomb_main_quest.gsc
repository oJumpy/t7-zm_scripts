...
{
	/#
		level util::waittill_any("Dev Block strings are not supported", "Dev Block strings are not supported");
		self.has_vinyl = 1;
...
function staff_unlock_with_zone_capture(s_staff_piece)
{
	...
	foreach(uts_box in level.a_uts_challenge_boxes)
	{
		if(uts_box.str_location == "church_capture")
		{
			...
			level thread namespace_a528e918::open_box(undefined, uts_box, &reward_staff_piece);
			return;
		}
	}
}
...
function reward_staff_piece(player, s_stat)
{
	...
	util::wait_network_frame();
	if(!namespace_a528e918::reward_rise_and_grab(m_piece, 50, 2, 2, -1))
	{
		return 0;
	}
	n_dist = 9999;
...
function staff_charger_get_player_msg(e_player)
{
	...
	else if(e_player HasWeapon(self.stub.staff_data.w_weapon))
	{
	...
	/#
		...
		while(self.charger.is_inserted)
		{
			if(self.charger.is_charged)
			{
				...
				print3d(self.origin, self.prev_ammo_stock + "Dev Block strings are not supported" + maxAmmo, VectorScale((1, 1, 1), 255), 1);
			}
			else
			{
				print3d(self.origin, self.charger.charges_received + "Dev Block strings are not supported" + 20, VectorScale((1, 1, 1), 255), 1);
			}
			wait(0.05);
		}
	#/
