...
function init()
{
	...
	else if(!isdefined(level.attack_player_thru_boards_range))
	{
	...
	/#
		level thread function_fa618012();
	#/
}

/*
	Name: function_fa618012
	Namespace: zm_spawner
	Checksum: 0x7F91A350
	Offset: 0x12C8
	Size: 0x467
	...
	Flags: None
*/
function function_fa618012()
{
	/#
		while(1)
		{
			if(isdefined(level.var_2e8ab4b9) && level.var_2e8ab4b9)
			{
				color = (1, 1, 1);
				var_bd0b4a56 = (1, 0, 0);
				var_60710385 = (0, 0, 1);
				...
				{
					foreach(location in zone.a_loc_types["Dev Block strings are not supported"])
					{
				...
				foreach(goal in level.exterior_goals)
				{
					if(isdefined(goal.var_b5486dd2) && goal.var_b5486dd2)
					{
						RecordCircle(goal.origin, 16, color);
						continue;
					}
					if(isdefined(goal.script_string) && goal.script_string == "Dev Block strings are not supported")
					{
						RecordCircle(goal.origin, 16, var_60710385);
						continue;
					}
					RecordCircle(goal.origin, 16, var_bd0b4a56);
				}
			}
			wait(0.05);
		}
...
function zombie_think()
{
	...
	/#
		self.var_5029f747 = self.first_node;
		self thread zm_puppet::function_ec1c999e();
	#/
	...
	if(isdefined(self.entrance_nodes))
	{
		for(i = 0; i < self.entrance_nodes.size; i++)
		{
			if(self zombie_bad_path())
			{
				...
				/#
					println("Dev Block strings are not supported" + self.origin + "Dev Block strings are not supported" + self.entrance_nodes[i].origin);
				#/
...
function zombie_pathing_get_breadcrumb(origin, breadcrumbs, bad_crumbs, pick_random)
{
	...
	/#
		if(pick_random)
		{
			zm_utility::debug_print("Dev Block strings are not supported");
		}
	#/
...
function do_zombie_spawn()
{
	...
	/#
		if(GetDvarInt("Dev Block strings are not supported"))
		{
			...
			self.var_7d932293 = spot.zone_name;
	...
	/#
		...
		{
			debugstar(spot.origin, GetDvarInt("Dev Block strings are not supported"), (0, 1, 0));
			host_player = util::getHostPlayer();
			Distance = Distance(spot.origin, host_player.origin);
			iprintln("Dev Block strings are not supported" + Distance / 12 + "Dev Block strings are not supported");
		}
	#/
...
function draw_zone_spawned_from()
{
	/#
		...
		while(1)
		{
			print3d(self.origin + VectorScale((0, 0, 1), 64), self.var_7d932293, (1, 1, 1));
			wait(0.05);
		}
	#/
}
...
function zombie_complete_emerging_into_playable_area()
{
	...
	if(isdefined(self.var_85ae7f15))
	{
		self SetGoal(self.var_85ae7f15);
		self.var_85ae7f15 = undefined;
	}
