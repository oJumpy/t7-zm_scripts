...
function updateDevSettings()
{
	/#
		...
		if(!isdefined(level.show_spawns) || level.show_spawns != show_spawns)
		{
			...
			SetDvar("Dev Block strings are not supported", level.show_spawns);
		...
		if(!isdefined(level.show_start_spawns) || level.show_start_spawns != show_start_spawns)
		{
			...
			SetDvar("Dev Block strings are not supported", level.show_start_spawns);
		...
		if(level.players.size > 0)
		{
			...
			{
				level thread devgui_increase_momentum(GetDvarInt("Dev Block strings are not supported"));
				SetDvar("Dev Block strings are not supported", "Dev Block strings are not supported");
			}
			else if(GetDvarString("Dev Block strings are not supported") != "Dev Block strings are not supported")
			{
			...
			else if(GetDvarString("Dev Block strings are not supported") != "Dev Block strings are not supported")
			{
				...
				if(level.gametype == "Dev Block strings are not supported" && isdefined(level.radios))
				{
					if(!level.devgui_show_hq)
					{
						...
						{
							color = (1, 0, 0);
							level showOneSpawnPoint(level.radios[i], color, "Dev Block strings are not supported", 32, "Dev Block strings are not supported");
						}
					}
					else
					{
		...
		if(GetDvarString("Dev Block strings are not supported") != "Dev Block strings are not supported")
		{
			...
			specialties = StrTok(perk, "Dev Block strings are not supported");
			players = GetPlayers();
			iprintln("Dev Block strings are not supported" + perk + "Dev Block strings are not supported");
			for(i = 0; i < players.size; i++)
			{
	...
	/#
		...
		for(;;)
		{
			...
			foreach(player in players)
			{
				...
				if(player.sessionstate != "Dev Block strings are not supported")
				{
					continue;
				}
...
function devgui_increase_momentum(score)
{
	/#
		...
		foreach(player in players)
		{
			...
			player globallogic_score::givePlayerMomentumNotification(score, &"Dev Block strings are not supported", "Dev Block strings are not supported", 0);
		}
	#/
}
...
function xKillsY(attackerName, victimName)
{
	/#
		...
		victim thread [[level.callbackPlayerDamage]](attacker, attacker, 1000, 0, "Dev Block strings are not supported", level.weaponNone, (0, 0, 0), (0, 0, 0), "Dev Block strings are not supported", 0, 0);
	#/
}
...
function showSpawnpoints()
{
	/#
		if(isdefined(level.Spawnpoints))
		{
			...
			{
				showOneSpawnPoint(level.Spawnpoints[spawn_point_index], color, "Dev Block strings are not supported");
			}
		}
		for(i = 0; i < level.dem_spawns.size; i++)
		{
			color = (0, 1, 0);
			showOneSpawnPoint(level.dem_spawns[i], color, "Dev Block strings are not supported");
		}
		return;
	#/
}
...
function showStartSpawnpoints()
{
	/#
		...
		team_colors = [];
		team_colors["Dev Block strings are not supported"] = (1, 0, 1);
		team_colors["Dev Block strings are not supported"] = (0, 1, 1);
		team_colors["Dev Block strings are not supported"] = (1, 1, 0);
		team_colors["Dev Block strings are not supported"] = (0, 1, 0);
		team_colors["Dev Block strings are not supported"] = (0, 0, 1);
		team_colors["Dev Block strings are not supported"] = (1, 0.7, 0);
		team_colors["Dev Block strings are not supported"] = (0.25, 0.25, 1);
		team_colors["Dev Block strings are not supported"] = (0.88, 0, 1);
		foreach(team in level.teams)
		{
			color = team_colors[team];
			foreach(spawnpoint in level.spawn_start[team])
			{
				showOneSpawnPoint(spawnpoint, color, "Dev Block strings are not supported");
			}
		}
		return;
	#/
