...
function moon_push_zombies_when_astro_explodes(position)
{
	...
	self thread namespace_ddd35ff::quantum_bomb_zombie_fling_result(position);
}
...
function setup_fields()
{
	...
	foreach(str_exploder in level.var_2f9ab492)
	{
...
{
	level thread namespace_359e846b::hack_wallbuys();
	level thread namespace_75cc53cb::hack_perks();
	level thread namespace_b03b0164::hack_packapunch();
	level thread namespace_b1d3fbb7::hack_boards();
	level thread namespace_65fac977::hack_doors("zombie_airlock_buy", &zm_moon_utility::moon_door_opened);
	level thread namespace_65fac977::hack_doors();
	level thread namespace_27e18f93::hack_powerups();
...
function packapunch_hack_think()
{
	...
	while(1)
	{
		...
		namespace_6d813654::register_pooled_hackable_struct(level._pack_hack_struct, &namespace_b03b0164::packapunch_hack);
	}
}
...
function assign_lowest_unused_character_index()
{
	...
	else if(!function_5c35365f(players))
	{
		return charindexarray[2];
	}
...
function offhand_weapon_give_override(str_weapon)
{
	...
	{
		self namespace_ddd35ff::player_give_quantum_bomb();
		self zm_weapons::play_weapon_vo(str_weapon);
		return 1;
	}
	return 0;
		...
		switch(cmd_strings[0])
		{
			case "Dev Block strings are not supported":
			{
				...
				for(i = 0; i < players.size; i++)
				{
					...
					if(entnum == 2)
					{
						players[i] zm_sidequests::add_sidequest_icon("Dev Block strings are not supported", "Dev Block strings are not supported");
			...
			case "Dev Block strings are not supported":
			{
				...
				foreach(player in players)
				{
					if(player HasWeapon(level.var_17bac01d))
					{
						...
						player zm_utility::set_player_tactical_grenade("Dev Block strings are not supported");
						player notify("starting_quantum_bomb");
					}
				}
				Array::thread_all(GetPlayers(), &namespace_d6b63386::player_give_black_hole_bomb);
				break;
			}
			case "Dev Block strings are not supported":
			{
				...
				Array::thread_all(GetPlayers(), &namespace_ddd35ff::player_give_quantum_bomb);
				break;
			}
			case "Dev Block strings are not supported":
			{
				trigger = GetEnt("Dev Block strings are not supported", "Dev Block strings are not supported");
			...
			{
				players = GetPlayers();
				Teleporter = GetEnt("Dev Block strings are not supported", "Dev Block strings are not supported");
				for(i = 0; i < players.size; i++)
				{
			...
			case "Dev Block strings are not supported":
			{
				zm_moon_digger::digger_activate("Dev Block strings are not supported");
				break;
			}
			case "Dev Block strings are not supported":
			{
			...
			case "Dev Block strings are not supported":
			{
				level.digger_speed_multiplier = GetDvarFloat("Dev Block strings are not supported");
...
function moon_digger_respawn(revivee)
{
	...
	if(level.zones["airlock_east2_zone"].is_enabled)
	{
		for(i = 0; i < spawn_points.size; i++)
		{
...
function blackhole_bomb_in_invalid_area(grenade, model, player)
{
	...
	{
		level thread namespace_d6b63386::black_hole_bomb_stolen_by_sam(player, model);
		return 1;
	}
	else
	{
