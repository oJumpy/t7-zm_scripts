...
function get_stat(str_stat, player)
{
	...
	/#
		Assert(isdefined(s_parent_stat), "Dev Block strings are not supported" + str_stat + "Dev Block strings are not supported");
	#/
	/#
		Assert(s_parent_stat.b_team || isdefined(player), "Dev Block strings are not supported" + str_stat + "Dev Block strings are not supported");
	#/
...
function watch_devgui_award_challenges()
{
	/#
		while(1)
		{
			n_award_challenge = GetDvarInt("Dev Block strings are not supported");
