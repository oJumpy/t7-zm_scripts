...
function quadrotor_medallions()
{
	...
	for(b_retrieved = 0; !b_retrieved;  = 0)
	{
...
function swap_mg(e_player)
{
	...
	if(zombie_utility::is_player_valid(e_player) && !e_player.IS_DRINKING && !e_player bgb::is_enabled("zm_bgb_disorderly_combat") && !zm_utility::is_placeable_mine(var_1bcd223d) && !zm_equipment::is_equipment(var_1bcd223d) && "none" != var_1bcd223d.name && !e_player zm_equipment::hacker_active() && (!isdefined(level.revive_tool) || (isdefined(level.revive_tool) && level.revive_tool != var_1bcd223d)))
	{
		...
		else
		{
			...
			{
				e_player TakeWeapon(var_1bcd223d);
			}
...
function onplayerconnect_ee_jump_scare()
{
	...
	while(!level.b_saw_jump_scare)
	{
		...
		if(n_time >= 25 && self AdsButtonPressed() && self GetCurrentWeapon().isSniperWeapon && self zm_zonemgr::entity_in_zone("zone_nml_18") && zm_utility::is_player_looking_at(level.jump_scare_lookat_point.origin, 0.998, 0, undefined))
		{
			...
			for(n_time = 0; self AdsButtonPressed() && n_time < 5; n_time++)
			{
				wait(0.05);
			}
...
function bunker_volume_death_check()
{
	...
	/#
		Assert(isdefined(volume), volume_name + "Dev Block strings are not supported");
	#/
	...
	if(isdefined(attacker) && isPlayer(attacker))
	{
		if(attacker.sq_one_inch_punch_stage == 5 && (self.damageMod == "MOD_MELEE" || self.damageWeapon.name == "tomb_shield"))
		{
			if(self istouching(volume))
			{
				...
				/#
					iprintln("Dev Block strings are not supported" + attacker.sq_one_inch_punch_kills);
				#/
...
function radio_ee_debug()
{
	/#
		...
		while(1)
		{
			print3d(self.origin, "Dev Block strings are not supported", VectorScale((1, 0, 1), 255), 1);
			wait(0.05);
		}
	#/
}
