...
function move_zeppelin_down_new_path(a_structs)
{
	...
	{
		/#
			Assert(isdefined(s_path_start.target), "Dev Block strings are not supported" + s_path_start.origin + "Dev Block strings are not supported");
		#/
		s_path_start.goal_struct = struct::get(s_path_start.target, "targetname");
		/#
			Assert(isdefined(s_path_start.goal_struct), "Dev Block strings are not supported" + s_path_start.origin);
		#/
	}
