...
function quantum_bomb_select_result(position)
{
	...
	/#
		result_name = GetDvarString("Dev Block strings are not supported");
		if(result_name != "Dev Block strings are not supported" && isdefined(level.quantum_bomb_results[result_name]))
		{
...
function quantum_bomb_random_weapon_starburst_result(position)
{
	self thread zm_audio::create_and_play_dialog("kill", "quant_good");
	/#
		starburst_debug = GetDvarInt("Dev Block strings are not supported");
		if(starburst_debug)
		{
