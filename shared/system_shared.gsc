...
function register(str_system, func_preinit, func_postinit, reqs)
{
	...
	{
		/#
			ASSERTMSG("Dev Block strings are not supported" + str_system + "Dev Block strings are not supported");
		#/
		return;
	}
...
function exec_post_system(req)
{
	/#
		...
		{
			/#
				ASSERTMSG("Dev Block strings are not supported" + req + "Dev Block strings are not supported");
			#/
		}
	#/
...
function run_post_systems()
{
	...
	{
		/#
			Assert(func.predone || func.Ignore, "Dev Block strings are not supported");
		#/
...
{
	/#
		Assert(!isdefined(level.gametype), "Dev Block strings are not supported");
	#/
	if(!isdefined(level.system_funcs) || !isdefined(level.system_funcs[str_system]))
	{
