...
function state_combat_update(params)
{
	...
	while(isdefined(self.inpain) && self.inpain)
	{
		wait(0.1);
		continue;
		if(!isdefined(self.enemy) || (isdefined(self.raps_force_patrol_behavior) && self.raps_force_patrol_behavior))
		{
			...
			foreach(point in queryResult.data)
			{
				/#
					...
					point._scoreDebug["Dev Block strings are not supported"] = mapfloat(0, 200, 0, 100, point.distToOrigin2D);
				#/
				...
				/#
					...
					point._scoreDebug["Dev Block strings are not supported"] = RandomFloatRange(0, 50);
				#/
				...
				if(isdefined(self.prevMoveDir))
				{
					...
					if(VectorDot(moveDir, self.prevMoveDir) > 0.64)
					{
						/#
							...
							point._scoreDebug["Dev Block strings are not supported"] = RandomFloatRange(50, 150);
						#/
		...
		else if(!self.enemy.allowdeath && self GetPersonalThreatBias(self.enemy) == 0)
		{
...
function jump_detonate()
{
	...
	self.is_jumping = 1;
	wait(0.4);
	for(time_to_land = 0.6; time_to_land > 0;  = 0.6)
	{
