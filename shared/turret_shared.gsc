...
function _get_default_target_offset(e_target, n_index)
{
	...
	if(s_turret.str_weapon_type == "bullet")
	{
		if(isdefined(e_target))
		{
			if(IsSentient(self) && IsSentient(e_target))
			{
				...
				else if(isdefined(s_turret.n_torso_targetting_offset))
				{
				}
				else
				{
	...
	if(isdefined(s_turret.n_target_leading_factor))
	{
	}
	else if(0 != 0 && isdefined(e_target) && IsSentient(self) && IsSentient(e_target) && !isVehicle(e_target))
	{
...
function fire(n_index)
{
	s_turret = _get_turret_data(n_index);
	/#
		Assert(isdefined(n_index) && n_index >= 0, "Dev Block strings are not supported");
	#/
	if(n_index == 0)
	{
...
function fire_for_time(n_time, n_index)
{
	...
	/#
		Assert(isdefined(n_time), "Dev Block strings are not supported");
	#/
	self endon("death");
	...
	/#
		...
		/#
			Assert(n_time >= w_weapon.fireTime, "Dev Block strings are not supported" + n_time + "Dev Block strings are not supported" + w_weapon.fireTime);
		#/
	#/
...
{
	/#
		Assert(isdefined(e_target), "Dev Block strings are not supported");
	#/
	self endon("drone_death");
	self endon("death");
	s_turret = _get_turret_data(n_index);
...
{
	if(isdefined(self.waittill_turret_on_target_delay))
	{
	}
	do
	{
...
function _debug_turret_think(n_index)
{
	/#
		self endon("death");
		self endon("Dev Block strings are not supported" + _index(n_index));
		self endon("Dev Block strings are not supported" + _index(n_index));
		s_turret = _get_turret_data(n_index);
		v_color = (0, 0, 1);
		while(1)
		{
			...
			{
				str_team = "Dev Block strings are not supported";
			}
			str_target = "Dev Block strings are not supported";
			e_target = s_turret.e_next_target;
			if(isdefined(e_target))
			{
				if(IsActor(e_target))
				{
					str_target = str_target + "Dev Block strings are not supported";
				}
				else if(isPlayer(e_target))
				{
					str_target = str_target + "Dev Block strings are not supported";
				...
				else if(isdefined(e_target.targetname) && e_target.targetname == "Dev Block strings are not supported")
				{
					str_target = str_target + "Dev Block strings are not supported";
				}
			...
			str_debug = self GetEntNum() + "Dev Block strings are not supported" + str_team + "Dev Block strings are not supported" + str_target;
			Record3DText(str_debug, self.origin, v_color, "Dev Block strings are not supported", self);
			wait(0.05);
		}
	#/
}
...
function _is_target_within_range(e_target, s_turret)
{
	if(isdefined(s_turret.n_max_target_distance_squared) || isdefined(s_turret.n_min_target_distance_squared))
	{
		...
		if(isdefined(s_turret.n_max_target_distance_squared))
		{
		}
		else if(n_dist_squared > 811711611)
		{
			return 0;
		}
		if(isdefined(s_turret.n_min_target_distance_squared))
		{
		}
		else if(n_dist_squared < 0)
		{
			return 0;
		}
	}
...
function can_hit_target(e_target, n_index)
{
	...
	b_target_in_view = is_target_in_view(e_target.origin + v_offset, e_target GetTagOrigin("tag_eye"));
	b_trace_passed = 1;
	if(b_target_in_view)
	{
		if(!s_turret.b_ignore_line_of_sight)
		{
			...
			b_trace_passed = trace_test(e_target, s_turret.n_torso_targetting_offset - (0, 0, 0), 0);
		}
