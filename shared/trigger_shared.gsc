...
function get_trigger_look_target()
{
	if(isdefined(self.target))
	{
		...
		{
			/#
				Assert(a_targets.size == 1, "Dev Block strings are not supported" + self.origin + "Dev Block strings are not supported");
			#/
...
function trigger_spawner(trigger)
{
	...
	/#
		Assert(a_spawners.size > 0, "Dev Block strings are not supported" + trigger.origin + "Dev Block strings are not supported");
	#/
	trigger endon("death");
	trigger wait_till();
...
function friendly_respawn_trigger(trigger)
{
	...
	/#
		Assert(spawners.size == 1, "Dev Block strings are not supported" + trigger.target + "Dev Block strings are not supported");
	#/
	spawner = spawners[0];
	/#
		Assert(!isdefined(spawner.script_forceColor), "Dev Block strings are not supported" + spawner.origin + "Dev Block strings are not supported");
	#/
	spawners = undefined;
	spawner endon("death");
	while(1)
...
function trigger_once(trig)
{
	...
	if(isdefined(trig))
	{
		/#
			...
			println("Dev Block strings are not supported" + trig GetEntityNumber() + "Dev Block strings are not supported" + trig.origin);
			println("Dev Block strings are not supported");
		#/
		trig delete();
	}
...
{
	/#
		Assert(isdefined(trigger.script_hint), "Dev Block strings are not supported" + trigger.origin + "Dev Block strings are not supported");
	#/
	trigger endon("death");
	...
	waittillframeend;
	/#
		Assert(isdefined(level.trigger_hint_string[trigger.script_hint]), "Dev Block strings are not supported" + trigger.script_hint + "Dev Block strings are not supported");
	#/
	trigger waittill("trigger", other);
	/#
		Assert(isPlayer(other), "Dev Block strings are not supported");
	#/
...
function wait_till(str_name, str_key, e_entity, b_assert)
{
	...
	if(isdefined(str_name))
	{
		...
		{
			Assert(!b_assert || triggers.size > 0, "Dev Block strings are not supported" + str_name + "Dev Block strings are not supported" + str_key);
		}
		/#
		#/
		if(triggers.size > 0)
...
function _trigger_wait(e_entity)
{
	...
	/#
		...
		{
			/#
				Assert(!IsArray(e_entity), "Dev Block strings are not supported");
			#/
			continue;
		}
		if(self.classname === "Dev Block strings are not supported")
		{
			/#
				Assert(!IsArray(e_entity), "Dev Block strings are not supported");
			#/
...
function use(str_name, str_key, ent, b_assert)
{
	...
	if(isdefined(str_name))
	{
		...
		if(!isdefined(e_trig))
		{
			...
			{
				/#
					ASSERTMSG("Dev Block strings are not supported" + str_name + "Dev Block strings are not supported" + str_key);
				#/
			}
			return;
		}
