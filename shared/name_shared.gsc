...
function add_nationality_names(str_nationality)
{
	switch(str_nationality)
	{
		...
		{
			/#
				ASSERTMSG("Dev Block strings are not supported" + str_nationality);
			#/
			break;
		}
	}
...
function add_override_name_func(nationality, func)
{
	...
	/#
		Assert(!isdefined(level._override_name_funcs[nationality]), "Dev Block strings are not supported");
	#/
...
function getRankFromName(name)
{
	...
	switch(shortRank)
	{
		...
		{
			/#
				println("Dev Block strings are not supported" + shortRank + "Dev Block strings are not supported");
			#/
			self.airank = "private";
			break;
		}
...
{
	/#
		Assert(str_search_array.size, "Dev Block strings are not supported");
	#/
